...
def collect_us_valuations(rows: Iterable[MarketRow]) -> Dict[str, ValuationMetrics]:
    ...
    for row in rows:
        ...
        metrics = ValuationMetrics(
            ...
            fcf_yield_pct=fcf_yield(free_cash_flow, market_cap),
...
def sample_valuations(rows: List[MarketRow]) -> Dict[str, ValuationMetrics]:
    defaults = {
        "NVDA": ValuationMetrics("NVDA", trailing_pe=48.0, forward_pe=32.0, price_to_book=35.0, ev_to_sales=26.0, profit_margin_pct=55.0, roe_pct=72.0, free_cash_flow=60_000_000_000, fcf_yield_pct=1.5, summary="", source="sample"),
        "AAPL": ValuationMetrics("AAPL", trailing_pe=29.0, forward_pe=25.0, price_to_book=38.0, ev_to_sales=7.5, profit_margin_pct=24.0, roe_pct=150.0, free_cash_flow=100_000_000_000, fcf_yield_pct=3.2, summary="", source="sample"),
        "TSLA": ValuationMetrics("TSLA", trailing_pe=65.0, forward_pe=55.0, price_to_book=9.0, ev_to_sales=6.0, profit_margin_pct=8.0, roe_pct=12.0, free_cash_flow=4_000_000_000, fcf_yield_pct=0.8, summary="", source="sample"),
        "AMD": ValuationMetrics("AMD", trailing_pe=120.0, forward_pe=34.0, price_to_book=4.5, ev_to_sales=8.0, profit_margin_pct=4.0, roe_pct=3.0, free_cash_flow=1_300_000_000, fcf_yield_pct=0.5, summary="", source="sample"),
        "PLTR": ValuationMetrics("PLTR", trailing_pe=90.0, forward_pe=70.0, price_to_book=16.0, ev_to_sales=25.0, profit_margin_pct=18.0, roe_pct=10.0, free_cash_flow=900_000_000, fcf_yield_pct=1.4, summary="", source="sample"),
...
def with_summary(row: MarketRow, metrics: ValuationMetrics) -> ValuationMetrics:
    ...
    return ValuationMetrics(
        ...
        fcf_yield_pct=metrics.fcf_yield_pct,
...
def summarize_valuation(row: MarketRow, metrics: ValuationMetrics) -> str:
    ...
    if fcf_yield_pct is not None:
        if fcf_yield_pct >= 4:
            notes.append("FCF 수익률은 매력적인 편")
        elif fcf_yield_pct <= 1:
            notes.append("FCF 수익률은 낮은 편")
...
def fcf_yield(free_cash_flow: Optional[float], market_cap: Optional[float]) -> Optional[float]:
